...
<table>
					...
					<tr>
						...
						ModelDane ww1=ud2.findByIdmodel(c.getIDMODEL().getIDMODEL());
						%>
						<td><%=w1.getNazwa()%></td>
						<td><%=ww1.getModel()%></td>
						<td><img src="<%=ww1.getLink()%>" width="120" height="120"/></td>
						<td><%=ud.getNazwa()%></td>
						<td><img src="<%=ud.getObraz()%>" width="120" height="120"/></td>
						<td><%=ud.getIlosc()%></td>
						<td><%=ud.getCena()+" zł"%></td>
					<td><a class="more"
							href="RezerCzesc?idczesc=<%=c.getIDCZESC()%>">Rezerwacja</a></td>
